...
contract AxiePresaleExtended is HasNoContracts, Pausable {
  ...
  uint256 constant public INITIAL_PRICE_INCREMENT = 0.000016 ether; // 0.0016 Ether
  ...
  modifier whenNotInitialized {
    require(address(presaleContract) == address(0));
    _;
  }
    ...
    whenNotPaused
  {
    ...
    address _actualReferrer = address(0);
    ...
    returns (uint256)
  {
    ...
    emit RefCreditsMinted(_receiver, _numMintedCredits);
    ...
    returns (uint256 _remainingQuantity)
  {
    ...
    if (_quantity > 0) {
      ...
      emit RewardedAxiesRedeemed(_receiver, _quantity);
    }
  }
    ...
    )
  {
    ...
    for (uint256 i = 0; i < _quantity; i++) {
      ...
      if (_nextPrice / 0.1 ether != _currentPrice / 0.1 ether) {
        _currentIncrement >>= 1;
      }
  ...
  function _applyRefCredits(address _receiver, uint256 _numAppliedCredits) private {
    ...
    if (_numRewards > 0) {
      ...
      emit AxiesRewarded(_receiver, _numRewards);
    }
  }
    ...
    returns (uint256 _remainingQuantity)
  {
    ...
    if (_quantity > 0) {
      ...
      emit AdoptedAxiesRedeemed(_receiver, _class, _quantity);
    }
  }
}